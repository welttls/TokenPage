...
### 修复
- **OpenCode Go 漏抓 MiMo 族**：页面各表模型显示名写法不一致——价格表用空格（`MiMo V2.5`）、模型清单表用连字符（`MiMo-V2.5`），抓取器用价格表名字对清单表 `name_to_id` 精确匹配失败 → MiMo 整族落入 `skipped_names` 被跳过。新增名称归一化匹配（去空格/连字符/下划线、转小写），命中后统一改用清单表展示名，保证价格、模型 ID、ZDR 三者对齐

### 效果
- MiMo 族订阅服务恢复收录：`mimo-v2.5`（$0.14/$0.28，额度×6）与 `mimo-v2.5-pro`（$0.435/$0.87，额度×1.5）正常入库并显示于 Web 矩阵，OpenCode Go 订阅条目 18 → 20 条
